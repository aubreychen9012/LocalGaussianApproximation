...
class covVAEModel():
    ...
    def build_network(self):
        ...
        guessed_z = self.z_mean + tf.matmul(samples, self.z_A_T)
        ...
        decoder_err_mu = tf.zeros_like(self.decoder_output)
        # for i in range(25):
        #     samples = tf.random_normal(tf.shape(self.z_mean), 0., 1., dtype=tf.float32)
        #     guessed_z = self.z_mean + tf.matmul(samples, self.z_A_T)
        #     # guessed_z = z_mean + (z_A_T * samples)
        #     self.decoder_output, self.dec_embed = \
        #         self.model.decoder(guessed_z, name="img", is_train=True, reuse=True)
        #     err = (self.decoder_output - self.image_matrix)**2
        #     #err = tf.abs(self.decoder_output - self.image_matrix)
        #     decoder_err_mu += err
        # decoder_err_mu = tf.sqrt(decoder_err_mu / 24.)
        # self.mu_var = decoder_err_mu
        # self.variance_loss = losses.l1loss(self.res, decoder_err_mu)
        ...
        # for i in range(25):
        #     samples = tf.random_normal(tf.shape(self.z_mean), 0., 1., dtype=tf.float32)
        #     guessed_z = z_mean_valid + tf.matmul(samples, z_A_T_valid)
        #     # guessed_z = z_mean + (z_A_T * samples)
        #     self.decoder_output_test, _ = \
        #         self.model.decoder(guessed_z, name="img", is_train=True, reuse=True)
        #     err = (self.decoder_output_test - self.image_matrix)**2
        #     #err = tf.abs(self.decoder_output - self.image_matrix)
        #     decoder_err_mu += err
        # decoder_err_mu = tf.sqrt(decoder_err_mu / 24.)
        # self.mu_var_test = decoder_err_mu
        # self.variance_loss_test = losses.l1loss(self.res_test, decoder_err_mu)

        # self.var_loss = tf.reduce_mean(self.variance_loss)
        ...
        # self.variance_loss_test, self.mu_var_test = losses.aggregate_var_loss(self.decoder_output_test,
        #                                                    self.image_matrix,
        #                                                    self.res_test)
        # self.var_loss_test = tf.reduce_mean(self.variance_loss_test)
    ...
    def initialize(self):
        ...
        #vae_vars = [var for var in t_vars if 'predictor' not in var.name]
        stddev_vars = [var for var in t_vars if 'e_conv2' in var.name or
                       'e_conv3' in var.name or 'e_conv4' in var.name or
                       'e_conv5' in var.name or 'e_bn2' in var.name or
                       'e_bn3' in var.name or 'e_bn4' in var.name]
        #stddev_vars = [var for var in t_vars if "res" not in var.name and "e_conv1" not in var.name
        #               and "e_bn1" not in var.name and "decoder" not in var.name]
        with tf.device("/gpu:0"):
            update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            with tf.control_dependencies(update_ops):
                self.train_op = tf.train.AdamOptimizer(self.config["lr"]).minimize(self.loss)
                self.train_var_op = tf.train.AdamOptimizer(self.config["lr"]).minimize(self.var_loss,
                                                                                    var_list = stddev_vars)
    ...
    def var_estimate(self):
        self.pred_var = tf.placeholder('float32',
                                           [self.config["batch_size"], self.config["spatial_size"],
                                            self.config["spatial_size"], 1],
                                           name='input_var')
        self.variance_loss = losses.l1loss(self.res, self.pred_var)
        self.var_loss = tf.reduce_mean(self.variance_loss)

        self.variance_loss_test = losses.l1loss(self.res_test, self.pred_var)
        self.var_loss = tf.reduce_mean(self.variance_loss_test)


    def train_var(self, input_images, input_var):
        self.input_images = input_images.astype("float32")
        self.input_var = input_var
        # input_masks = input_masks.astype("float32")
        feed_dict = {self.image_matrix: input_images,
                     self.pred_var: input_var}
        self.sess.run(self.train_var_op, feed_dict)
        self.out_mu = self.sess.run(self.decoder_output, feed_dict)
        self.residual_output = self.sess.run(self.res, feed_dict)
    ...
    def validate_var(self, input_images, input_var):
        self.input_images_test = input_images.astype("float32")
        self.input_var_test = input_var
        # input_masks = input_masks.astype("float32")
        feed_dict = {self.image_matrix: input_images,
                     self.pred_var: input_var}
        #self.sess.run(self.train_var_op, feed_dict)
        self.out_mu_test = self.sess.run(self.decoder_output, feed_dict)
    ...
    def visualize(self, model_name, ep):
        ...
        plot_batch(self.input_var, model_name + '_samples/gt_var_' + str(ep) + '.png')
        ...
        plot_batch(self.input_var_test,
                   model_name + '_samples/test_gt_var_' + str(ep) + '.png')

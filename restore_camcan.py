...
class Restoration():
    ...
    def gaussian_approximator(self, weight, riter, prior_residuals,
        ...
        self.gfunc = tf.reduce_mean(self.autoencoder_l2_weighted)
        if "L1" in constraint:
        ...
        with tf.control_dependencies(update_ops):
            ...
            restore_op = adam.apply_gradients([(grad_vars[0][0] * self.img_mask, grad_vars[0][1])])
            #restore_op = adam.apply_gradients(
            #    [(tf.clip_by_value(grad_vars[0][0] * self.img_mask, -5, 5), grad_vars[0][1])])

        initialize_uninitialized(self.sess)
        for i in range(riter):
            restored_images = self.sess.run(self.img_ano)
            #prior_residuals = self.sess.run(self.pred_res)
    ...
    def gaussian_approximator_recons_var(self, weight, riter,
                              dec_mu, constraint='L1'):
        t_vars = tf.trainable_variables()
        update_vars = [var for var in t_vars if 'img_ano' in var.name]
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        ...
        for i in range(riter):
            ...
            if i % 100 == 0:
                ...
                # plot_batch(restored_mask, vis_dir + '/_masks_restored_' + str(i) + '.png')
        restored_images = self.sess.run(self.img_ano)
        pred_res = self.sess.run(self.pred_res, {self.res: decoded_std, self.res_mu: decoded_mu})
        return restored_images, pred_res


def restore_camcan(weight, log_dir, model_name, step, config, img_size, batch_size,
                   n_random_sub = 25, fprate = 0.,
                   method = 'neural',
                   constraint='L1',
                   renormalized = False):
    #if renormalized:
    ...
    data = h5py.File('/scratch_net/bmicdl01/Data/camcan_t1_val_set.hdf5')
    #else:
        ...
        for batch in tl.iterate.minibatches(inputs=res_exp, targets=mask_exp,
            ...
            if method =="neural":
                ...
                restored_images, pred_res_restored = \
                    image_restoration.gaussian_approximator(weight, 800, prior_residuals,
                                                            decoded_mu,
                ...
                image_restoration.load(log_dir, model_name, step)
                restored_images, pred_res_restored = \
                    image_restoration.gaussian_approximator_recons_var(weight, 800,
                                                            decoded_mu,
                                                            constraint=constraint)
                time_end = time.time()
            ...
            #restored_images, pred_res_restored = \
            #    image_restoration.gaussian_approximator_recons_var(weight, 800, decoded_std,
            #                                            decoded_mu)
    ...
    # #threshold1 = determine_threshold(err_arr, 0.008)
    # threshold2 = determine_threshold(err_arr, 0.005)
    # threshold3 = determine_threshold(err_arr, 0.001)
    return [np.mean(restored_err_list), thresholds, fprate]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--constraint', type=str, default=0.)
    ...
    try:
        os.makedirs("tests/" + str(model_name) + '_samples_camcan_t1/')
    except OSError:
        pass
    vis_dir = "tests/" + str(model_name) + '_samples_camcan_t1/'

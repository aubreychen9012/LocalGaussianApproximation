...
class VariationalAutoencoder():
    ...
    def encoder(self, x, reuse=False, is_train=True):
        ...
        with tf.variable_scope(self.model_name+"_encoder", reuse=reuse):
            ...
            res6_std = resblock_down(res5, gf_dim * 16, gf_dim * 32, "res6_std", reuse, is_train)
            res6_res = resblock_down(res5, gf_dim * 16, gf_dim * 32, "res6_res", reuse, is_train)
        ...
        return conv7_mean_flat, conv7_std_flat, conv7_mean_res
    ...
    def decoder(self, x, name, reuse=False, is_train=True):
        ...
        with tf.variable_scope(self.model_name+"_decoder_"+name, reuse=reuse):
            ...
            resp1 = resblock_up(z_develop, s64, batch_size, gf_dim*32, gf_dim * 16, "gresp1", reuse, is_train)

            # s32*s32
            res0 = resblock_up(resp1, s32, batch_size, gf_dim*32, gf_dim * 16, "gres0", reuse, is_train)

            #s16*s16
            # res0 = ResBlockUp(resp2, s16, batch_size, gf_dim * 16, "gres0", reuse, is_train)

            # s16*s16
            res1 = resblock_up(res0, s16, batch_size, gf_dim * 16, gf_dim * 8, "gres1", reuse, is_train)
            ...
            # s8*s8
            res2 = resblock_up(res1, s8, batch_size, gf_dim * 8, gf_dim * 4, "gres2", reuse, is_train)
            ...
            # s4*s4
            res3 = resblock_up(res2, s4, batch_size, gf_dim * 4, gf_dim * 2, "gres3", reuse, is_train)
            ...
            # s2*s2
            res4 = resblock_up(res3, s2, batch_size, gf_dim * 2, gf_dim, "gres4", reuse, is_train)

...
from ops.blocks import resblock_down, resblock_up, resblock_up_bilinear
...
class VariationalAutoencoder():
    ...
    def encoder(self, x, reuse=False, is_train=True):
        ...
        with tf.variable_scope(self.model_name+"_encoder", reuse=reuse):
            ...
            # s32*s32
            res6_mu = resblock_down(res5, gf_dim * 16, gf_dim *32, "res6_mu", reuse, is_train,
                                 act=False)
            res6_stddev = resblock_down(res5, gf_dim * 16, gf_dim * 32, "res6_std", reuse, is_train,
                                 act=False)
            # res6_mu = tf.layers.conv2d(res6, gf_dim * 64, (1,1), padding='same',
            #                                kernel_initializer=w_init,
            #                                bias_initializer=b_init,
            #                                trainable=True,
            #                                name = "res6_mu")
            # res6_stddev = tf.layers.conv2d(res6, gf_dim * 64, (1, 1), padding='same',
            #                            kernel_initializer=w_init,
            #                            bias_initializer=b_init,
            #                            trainable=True,
            #                            name="res6_stddev")
    ...
    def decoder(self, x, name, reuse=False, is_train=True):
        ...
        with tf.variable_scope(self.model_name+"_decoder_"+name, reuse=reuse):
            ...
            # conv2_std = tf.layers.conv2d(conv1, gf_dim/2, (3, 3), padding='same', kernel_initializer=w_init,
            #                              bias_initializer=b_init, trainable=True, name="g_conv2_std",
            #                              reuse=reuse)
            # conv2_std = tf.nn.leaky_relu(conv2_std, 0.2)
            # conv2_std = tf.layers.conv2d(conv2_std, gf_dim/4, (3,3), padding='same', kernel_initializer=w_init,
            #                              bias_initializer=b_init, trainable=True, name="g_conv3_std",
            #                              reuse=reuse)
            # conv2_std = tf.nn.leaky_relu(conv2_std, 0.2)
            # conv2_std = tf.layers.conv2d(conv2_std, 1, (3,3), padding='same', kernel_initializer=w_init,
            #                              bias_initializer=b_init, trainable=True, name="g_conv4_std",
            #                              reuse=reuse)
